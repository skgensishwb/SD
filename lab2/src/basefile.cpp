...
void Base32File::buildDecodingTable() {
    for (int i = 0; i < 256; i++) {
        m_decodingTable[i] = -1;
    }
    for (int i = 0; i < 32; i++) {
        m_decodingTable[(unsigned char)m_table[i]] = i;
    }
}
...
size_t Base32File::write(const void *buf, size_t nBytes) {
    ...
    for (size_t i = 0; i < nBytes; i++) {
        unsigned int currentByte = static_cast<unsigned char>(data[i]);
        m_writeBuf = (m_writeBuf << 8) | currentByte;
        m_writeBits += 8;

        while (m_writeBits >= 5) {
            unsigned int idx = (m_writeBuf >> (m_writeBits - 5)) & 0x1F;
            char c = m_table[idx];
            writeRaw(&c, 1);

            m_writeBits -= 5;
            m_writeBuf &= (1U << m_writeBits) - 1;
        }
...
size_t Base32File::read(void *buf, size_t maxBytes) {
    ...
    while (m_readBits >= 8 && decoded < maxBytes) {
        unsigned char byte = (m_readBuf >> (m_readBits - 8)) & 0xFF;
        out[decoded++] = (char)byte;
        m_readBits -= 8;
        m_readBuf &= (1U << m_readBits) - 1;
    }

    while (decoded < maxBytes) {
        char c;
        size_t r = readRaw(&c, 1);
        if (r == 0) {
            break;
        }

        int fiveBits = m_decodingTable[(unsigned char)c];
        if (fiveBits == -1) {
            break;
        }

        m_readBuf = (m_readBuf << 5) | fiveBits;
        m_readBits += 5;
...
Base32File::Base32File() 
    : BaseFile(), 
    m_writeBits(0), m_writeBuf(0),
    m_readBits(0), m_readBuf(0)
{
...
Base32File::Base32File(const char* filePath, const char* mode)
...
Base32File::~Base32File() {
    if (m_writeBits > 0 && m_file != nullptr) {
        unsigned int idx = (m_writeBuf << (5 - m_writeBits)) & 0x1F;
        char c = m_table[idx];
        writeRaw(&c, 1);
        m_writeBits = 0;
        m_writeBuf = 0;
